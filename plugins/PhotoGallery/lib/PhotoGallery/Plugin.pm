...
sub load_list_filters {
	...
	my $mt = MT->instance;
	my @cats = MT::Category->load({ blog_id => $mt->blog->id },{sort => 'label'});
	my $reg;
...
sub mode_manage {
	...
	if ($asset) {
	    ...
	    $row->{'thumb_url'} = File::Spec->catfile($app->static_path,"plugins","PhotoGallery","text-icon.gif");
	    $row->{'thumb_w'} = 142;
	    $row->{'thumb_h'} = 133;
	    $row->{'entry_id'} = $obj->id;
	}
...
sub load_asset_from_entry {
	...
	{ class => '*' }, 
	{ lastn => 1,
	  join => MT::ObjectAsset->join_on(undef, {
	      asset_id => \'= asset_id', object_ds => 'entry', object_id => $obj->id })
            }
        );    
    return $asset;
}
