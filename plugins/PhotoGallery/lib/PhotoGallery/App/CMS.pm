...
use MT::Util qw( format_ts encode_url dirify archive_file_for iso2ts ts2iso epoch2ts );
use MT::ConfigMgr;
...
sub update_timestamps {
	...
	while (my $e = $iter->next) {
		...
		asset_id => \$clause, object_ds => 'entry', object_id => $e->id })});
	    if (@assets) {
		my $a = $assets[0];
		my $file = $a->file_path();
		if (-r $file && -f $file) {
...
sub upload_photo {
	...
	$tmpl->param(date_taken_fmt => 
		     format_ts( $date_format, $exif_date, $app->blog, $app->user ? $app->user->preferred_language : undef ));
	$tmpl->param(date_today_fmt => 
