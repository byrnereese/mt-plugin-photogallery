...
sub create_album {
    ...
    MT::Gallery::Publisher->publish( Set => $set, User => $app->{user},
...
sub upload {
    ...
    my $full = MT::Gallery::Photo->make_unique_filename($set,
							$set->path . $fname);
    defined($fmgr->put_data($fh, $full, 'upload'))
        or return $app->error($fmgr->errstr);
    seek $fh, 0, 0;  ## Rewind filehandle.
    my $photo = $set->add_photo(
				{ filename => File::Basename::basename($full) },
            $fh
				) or return $app->error($set->errstr);
...
sub reg_file {
    ...
    $app->{no_print_body} = 1;
    $app->set_header('Content-Disposition' => 'attachment; filename=wizard.reg');
    $app->send_http_header('application/octet-stream; name=wizard.reg');
    $app->print(
        qq(Windows Registry Editor Version 5.00\r\n\r\n) .
