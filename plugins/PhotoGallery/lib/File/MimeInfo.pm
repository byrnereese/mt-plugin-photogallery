...
our @EXPORT_OK =
  qw(extensions describe globs inodetype mimetype_canon mimetype_isa);
our $VERSION = '0.15';
our $DEBUG;
...
sub mimetype {
    ...
    croak 'subroutine "mimetype" needs a filename as argument'
      unless defined $file;
    croak 'You should use File::MimeInfo::Magic to check open filehandles'
      if ref $file;
    return
         inodetype($file)
      || globs($file)
...
sub inodetype {
    ...
    if ( $t eq 'inode/directory' ) {    # compare devices to detext mount-points
        ...
        $t = 'inode/mount-point'
          unless ( stat $dir )[0] == $dev;      # compare devices
        return $t;
    }
...
sub globs {
    my $file = pop;
    croak 'subroutine "globs" needs a filename as argument'
      unless defined $file;
    ...
    if ( $file =~ /\.(\w+(\.\w+)*)$/ ) {
        ...
        while (@ext) {
            ...
            warn
"WARNING: wantarray behaviour of globs() will change in the future.\n"
              if wantarray;
            return wantarray
              ? ( $extension{$ext}, $ext )
              : $extension{$ext}
...
sub default {
    my $file = pop;
    croak 'subroutine "default" needs a filename as argument'
      unless defined $file;
